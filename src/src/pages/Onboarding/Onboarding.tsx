 ...
 * 3 pasos: sector, tamano de empresa, necesidad principal.
 * Guarda en Supabase tabla onboarding_data y redirige al dashboard.
 * Se muestra automaticamente si el usuario no ha completado el onboarding.
 */
          ...
          <div className={styles.stepContent}>
            ...
            <div className={styles.optionsGrid}>
              {SECTORS.map((s) => (
                <button
                  key={s}
                  className={[styles.option, data.sector === s ? styles.optionActive : ''].join(' ').trim()}
          ...
          <div className={styles.stepContent}>
            ...
            <div className={styles.optionsCol}>
              {COMPANY_SIZES.map((s) => (
                <button
                  key={s.value}
                  className={[styles.option, styles.optionWide, data.companySize === s.value ? styles.optionActive : ''].join(' ').trim()}
                  onClick={() => setData((d) => ({ ...d, companySize: s.value }))}
                >
                  {s.label}
                </button>
              ))}
          ...
          <div className={styles.stepContent}>
            ...
            <div className={styles.optionsCol}>
              {NEEDS.map((n) => (
                <button
                  key={n}
                  className={[styles.option, styles.optionWide, data.mainNeed === n ? styles.optionActive : ''].join(' ').trim()}
                  onClick={() => setData((d) => ({ ...d, mainNeed: n }))}
                >
                  {n}
                </button>
              ))}
            ...
            <div className={styles.stepActions}>
              ...
              <Button onClick={handleFinish} loading={saving} disabled={!data.mainNeed}>
                Ir al panel
              </Button>
            </div>
          </div>
